...
def sac(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var, ac_kwargs=dict(), seed=0,
    ...
    with tf.variable_scope('main'):
        ...
        qc1_var,  qc1_pi_var  = var_fn(x_ph, a_ph, pi, name='qc1_var', **ac_kwargs)
        qc2_var, qc2_pi_var = var_fn(x_ph, a_ph, pi, name='qc2_var', **ac_kwargs)   
    ...
    with tf.variable_scope('target'):
        ...
        _, qc1_pi_var_targ = var_fn(x2_ph, a_ph, pi2, name='qc1_var', **ac_kwargs)
        _, qc2_pi_var_targ = var_fn(x2_ph, a_ph, pi2, name='qc2_var', **ac_kwargs)
    ...
    if proc_id()==0:
        var_counts = tuple(count_vars(scope) for scope in 
                           ['main/pi', 'main/qr1', 'main/qr2', 'main/qc1', 'main/qc2', 'main/qc1_var', 'main/qc2_var', 'main'])
        print(('\nNumber of parameters: \t pi: %d, \t qr1: %d, \t qr2: %d, \t qc1: %d, \t qc2: %d, \t qc1_var: %d, \t qc2_var: %d, \t total: %d\n')%var_counts)
    ...
    # Expectation of Qc and Var(Qc)
    qc = 0.5 * (qc1 + qc2)
    qc_var = 0.5 * (qc1_var + qc2_var) + 0.5 * (qc1 - 0.5 * (qc1 + qc2)) ** 2 + 0.5 * (qc2 - 0.5 * (qc1 + qc2)) ** 2
    qc_pi = 0.5 * (qc1_pi + qc2_pi)
    qc_pi_targ = 0.5 * (qc1_pi_targ + qc2_pi_targ)
    qc_pi_var = 0.5 * (qc1_pi_var + qc2_pi_var) + 0.5 * (qc1_pi - 0.5 * (qc1_pi + qc2_pi)) ** 2 + 0.5 * (qc2_pi - 0.5 * (qc1_pi + qc2_pi)) ** 2
    qc_pi_var_targ = 0.5 * (qc1_pi_var_targ + qc2_pi_var_targ) + 0.5 * (qc1_pi_targ - 0.5 * (qc1_pi_targ + qc2_pi_targ)) ** 2 + 0.5 * (qc2_pi_targ - 0.5 * (qc1_pi_targ + qc2_pi_targ)) ** 2
    ...
    qc1_var_loss = 0.5 * tf.reduce_mean(qc1_var + qc_var_backup - 2 * ((qc1_var * qc_var_backup) ** 0.5))
    qc2_var_loss = 0.5 * tf.reduce_mean(qc2_var + qc_var_backup - 2 * ((qc2_var * qc_var_backup) ** 0.5))
    
    q_loss = qr1_loss + qr2_loss + qc1_loss + qc2_loss + qc1_var_loss + qc2_var_loss
    ...
    vars_to_get = dict(LossPi=pi_loss, LossQR1=qr1_loss, LossQR2=qr2_loss, LossQC1=qc1_loss, LossQC2=qc2_loss, LossQC1Var=qc1_var_loss,
                       LossQC2Var=qc2_var_loss, LossQ=q_loss,
                       QR1Vals=qr1, QR2Vals=qr2, QC1Vals = qc1, QC2Vals = qc2, QC1Var = qc1_var, QC2Var = qc2_var,
                       LogPi=logp_pi, PiEntropy=pi_entropy,
    ...
    for t in range(total_steps // num_procs()):
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            ...
            logger.log_tabular('QC1Var', with_min_and_max=True)
            logger.log_tabular('QC2Var', with_min_and_max=True)
            ...
            logger.log_tabular('LossQC1Var', average_only=True)
            logger.log_tabular('LossQC2Var', average_only=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='Safexp-PointGoal1-v0') #Safexp-PointGoal2-v0
