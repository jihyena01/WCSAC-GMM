...
class PrioritizedReplayBuffer:
    """
    A Prioritized Experience Replay Buffer for SAC agents.
    """

    def __init__(self, obs_dim, act_dim, size, alpha=0.6):
    ...
    def store(self, obs, act, rew, next_obs, done, cost, td_error=1.0):
        """
        Store a new experience with initial priority based on TD-error.
        """
        max_priority = self.priorities.max() if self.size > 0 else td_error
        ...
        self.priorities[self.ptr] = max_priority ** self.alpha
        self.ptr = (self.ptr + 1) % self.max_size
        self.size = min(self.size + 1, self.max_size)

    def sample_batch(self, batch_size=32, beta=0.4):
        """
    ...
    def update_priorities(self, idxs, td_errors):
        ...
        for idx, td_error in zip(idxs, td_errors):
            self.priorities[idx] = (abs(td_error) + EPS) ** self.alpha
...
def sac(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var, ac_kwargs=dict(), seed=0,
    ...
    # Experience buffer
    replay_buffer = PrioritizedReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size, alpha=alpha)
    ...
    if cost_constraint is not None:
        ...
        damp = 0  # No damping if cost_constraint is None
    ...
    if use_costs:
        if cost_constraint is None:
            ...
            cost_constraint = None if cost_lim is None else cost_lim * (1 - gamma ** max_ep_len) / (1 - gamma) / max_ep_len
    ...
    for t in range(total_steps // num_procs()):
        ...
        def update_prioritized_replay_buffer():
            """
            Update function for SAC with PER.
            """
            batch = replay_buffer.sample_batch(batch_size, beta=beta)
            feed_dict = {
                x_ph: batch["obs1"],
                x2_ph: batch["obs2"],
                a_ph: batch["acts"],
            ...
            td_errors, values, _ = sess.run(
                [q_backup - min_q_pi, vars_to_get, grouped_update], feed_dict
            )
    ...
    # Use `update_prioritized_replay_buffer` wherever the replay buffer is used
        if t > 0 and t % update_freq == 0:
            for j in range(update_freq):
                update_prioritized_replay_buffer()
